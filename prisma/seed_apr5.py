"""
Seed script — April 5, 2026 (Day 5).
Inserts: GodownStock, DailyCounterStock (C1/C2/C3),
         DailyCounterSummary, DailyExpense, CashRecord

No godown receipts. No permit received. No SP changes from Day 4.
C1 Bhatta=100 (vs 50 on Days 1-4). No PERMIT_RENT/BREAKAGE on this day.
C3 tally = -6.
"""
...
for cnum in [1, 2, 3]:
    ...
    for pid, d in stock_c[cnum]:
        rows.append((counter_id[cnum], pid, APR5,
...
gd_rows = [(shop_id, pid, APR5, gd['ob'], gd['rec'], gd['cb']) for pid, gd in stock_gd]
execute_values(cur, """
...
for pid, gd in stock_gd:
    ...
    for cnum, qty in [(1, gd['sc1']), (2, gd['sc2']), (3, gd['sc3'])]:
        if qty > 0:
            dist_rows.append((gsid, counter_id[cnum], qty, APR5))
if dist_rows:
    execute_values(cur, """
...
summaries = [
    (1, 'Ramesh', '42788', '4990', '912', '47778', '8610', '8750', '37910', '2030', '910',  '0'),
    (2, None,     '35775', '6370', '1045','42145', '5485', '5575', '36045', '1570', '1045', '0'),
    (3, None,     '38971', '5760', '929', '44731', '8205', '8295', '36935',  '430',  '929', '-6'),
]
...
expenses_data = [
    (1, 'TEA',        40,    False),
    (1, 'GOOGLE_PAY', 8610,  True),
    (1, 'BHATTA',     100,   False),
    (2, 'TEA',        40,    False),
    (2, 'GOOGLE_PAY', 5485,  True),
    (2, 'BHATTA',     50,    False),
    (3, 'TEA',        40,    False),
    (3, 'GOOGLE_PAY', 8205,  True),
    (3, 'BHATTA',     50,    False),
]
for cnum, category, amount, is_upi in expenses_data:
    cur.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s)
    """, (counter_id[cnum], APR5, category, str(amount), is_upi))

# ── CashRecord ─────────────────────────────────────────────────
print('▶ Inserting CashRecords...')
cash_records = [
    # C1 collection
    (1, 'COLLECTION', 'NOTE', 500, 58),
    (1, 'COLLECTION', 'NOTE', 200,  5),
    (1, 'COLLECTION', 'NOTE', 100, 50),
    (1, 'COLLECTION', 'NOTE',  50, 40),
    # C2 collection
    (2, 'COLLECTION', 'NOTE', 500, 38),
    (2, 'COLLECTION', 'NOTE', 200, 23),
    (2, 'COLLECTION', 'NOTE', 100, 91),
    (2, 'COLLECTION', 'NOTE',  50, 22),
    (2, 'COLLECTION', 'NOTE',  20, 60),
    # C3 collection
    (3, 'COLLECTION', 'NOTE', 500, 48),
    (3, 'COLLECTION', 'NOTE', 200, 45),
    (3, 'COLLECTION', 'NOTE', 100, 20),
    (3, 'COLLECTION', 'NOTE',  50, 20),
    # Drawer cash
    (1, 'DRAWER_CASH', 'COIN', 1, 2030),
    (2, 'DRAWER_CASH', 'COIN', 1, 1570),
    (3, 'DRAWER_CASH', 'COIN', 1,  430),
    # Tips cash
    (1, 'TIPS_CASH', 'COIN', 1,  910),
    (2, 'TIPS_CASH', 'COIN', 1, 1045),
    (3, 'TIPS_CASH', 'COIN', 1,  929),
]
for cnum, cat, dtype, dval, count in cash_records:
    cur.execute("""
        ...
        DO NOTHING
    """, (counter_id[cnum], APR5, cat, dtype, dval, count, dval * count))
...
print()
print('✓ April 5, 2026 seed complete.')
